...
func (f *multilineFinder) Find(ctx context.Context, w io.Writer, path string, regex *regexp.Regexp, nlines int) error {
...
func (f *multilineFinder) find(ctx context.Context) error {
	if *f.text == "" {
		return nil
	}
	text := *f.text + "\n"
...
func (f *multilineFinder) print(lineNumber, charNumber int, head, match, tail string) {
	path := ""
	if f.path != "" {
